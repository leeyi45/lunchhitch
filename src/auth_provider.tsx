...
export function AuthProvider({ children }: any) {
  ...
  React.useEffect(() => FIREBASE_AUTH.onIdTokenChanged(async (user) => {
    if (!user) {
      setContextObj({
        user: null,
        status: 'unauthenticated',
      ...
      const usernameMatch = user.email?.match(/(.+)@lunchhitch.firebaseapp.com/);

      if (!usernameMatch) throw new Error(); // TODO Error handling
      const username = usernameMatch[1];

      const emailResp = await fetch('/api/prisma?collection=userInfo&method=findFirst', {
        method: 'POST',
        body: JSON.stringify({
          where: {
            id: username,
